...
const handler = async (req, res) => {
  ...
  const URL = 'https://insights-api.newrelic.com/v1/accounts/2673149/query?nrql=SELECT%20*%20FROM%20Heartbeat'
 ...
 .then(response => {
    ...
    for( i=0; i<l; i++) {
      ...
      output.push({ machine_id: data[i].machine_id, state: data[i].state });
    }
